...
const HotelInfo = () => {
  ...
  const loadAccessibilityData = async() => {
    const resp = await fetch('https://b4egc7c83h.execute-api.us-east-1.amazonaws.com/Production/accessibilities');
    const jsonData = await resp.json();
    setAccessibilityData(jsonData);
  };

  const loadServicesData = async() => {
    const resp = await fetch('https://b4egc7c83h.execute-api.us-east-1.amazonaws.com/Production/services');
    const jsonData = await resp.json();
